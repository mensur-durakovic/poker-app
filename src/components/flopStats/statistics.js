...
const FilledDiv = styled.div`
  ...
  background-color: ${(props) => (props.fillColor ? props.fillColor : "rgba(26,126,21,1)")};
  display: flex;
  align-items: center;
      ...
      <div className="flop-stats-statistics-body">
          ...
          {statisticsDataUpper.map((s) => (
            <div className="flop-stats-statistics-body-upper-row" key={s.name}>
              ...
              <div className="flop-stats-statistics-body-upper-bar">
                <FilledDiv
                  filled={`${s.value}%`}  fillColor='rgba(27, 98, 211, 1)'
                >{`${s.value}%`}</FilledDiv>
              </div>
            </div>
          ))}
        ...
        {statisticsDataLower.map((s) => (
            <div className="flop-stats-statistics-body-upper-row" key={s.name}>
              ...
              <div className="flop-stats-statistics-body-upper-bar">
                <FilledDiv
                  filled={`${s.value}%`} fillColor='rgba(26,126,21,1)'
                >{`${s.value}%`}</FilledDiv>
              </div>
            </div>
          ))}
